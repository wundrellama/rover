...
if len(tables) != 53 or len(set(tables)) != 53:
    raise SystemExit(
        f"schema-test: FAIL - DDL has {len(tables)} tables, "
        f"{len(set(tables))} unique (want 53/53)"
    )
...
if fk_count != 56 or restrict_count != 56:
    raise SystemExit(
        f"schema-test: FAIL - DDL has {fk_count} FKs and "
        f"{restrict_count} explicit RESTRICT pairs (want 56/56)"
    )
print(
    "schema-test: PASS - DDL has 53 unique tables, "
    "56 explicit RESTRICT FKs, zero forward references"
)
PY
...
[ "${counts[0]}" -eq 53 ] ||
  fail "live Obelisk has ${counts[0]} relations (want 53)"
[ "${counts[2]}" -eq 58 ] ||
  fail "live Obelisk has ${counts[2]} FK metadata rows (want 58)"
...
[ "$restrict_delete" -eq 58 ] ||
  fail "live metadata has $restrict_delete RESTRICT deletes (want 58)"
